...
func (h *AssessmentHandler) UpsertCurrentAssessment(c *gin.Context) {
	...
	cohortType := strings.ToLower(strings.TrimSpace(c.PostForm("cohortType")))
	if cohortType == "" {
		cohortType = strings.ToLower(strings.TrimSpace(existing.CohortType))
	}
	if cohortType == "" {
		cohortType = "school"
	}
	if cohortType != "school" && cohortType != "college" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "cohortType must be school or college"})
		return
	}
