...
func (h *SubmissionHandler) UploadSubmission(c *gin.Context) {
	...
	if err != nil {
		...
		} else if errors.Is(err, errInvalidAssessmentID) {
			status = http.StatusBadRequest
		}
		c.JSON(status, gin.H{"error": "assessment not available"})
		return
	}
	...
	questionMime := assessment.QuestionPaper.MimeType
	if questionMime == "" {
	...
	answerMime := assessment.AnswerKey.MimeType
	if answerMime == "" {
		sample := answerBytes
		if len(sample) > 512 {
			sample = sample[:512]
	...
	questionName := assessment.QuestionPaper.FileName
	if questionName == "" {
		questionName = filepath.Base(assessment.QuestionPaper.StoredPath)
	}
	answerName := assessment.AnswerKey.FileName
	if answerName == "" {
...
func (h *SubmissionHandler) resolveAssessment(parent context.Context, assessmentID string) (models.Assessment, error) {
	...
	filter := bson.M{"slug": currentAssessmentSlug}
	if assessmentID != "" {
