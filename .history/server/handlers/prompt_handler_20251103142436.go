...
func ensurePromptIndexes(col *mongo.Collection) {
    ...
    if _, err := col.Indexes().CreateOne(ctx, indexModel); err != nil {
        //nolint:forbidigo // initialization warning only
        println("warning: unable to ensure prompt index:", err.Error())
    }
}
...
func defaultPrompt() string {
    return "You are an experienced examiner. Analyse the provided answer script and award marks fairly. Always respond with a JSON object containing score, maxScore, and reasoning."
}
...
func modelToResponse(prompt models.Prompt) PromptResponse {
    ...
    updatedBy := ""
    if !prompt.UpdatedBy.IsZero() {
        updatedBy = prompt.UpdatedBy.Hex()
    }
