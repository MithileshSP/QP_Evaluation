...
func (h *SubmissionHandler) UploadSubmission(c *gin.Context) {
	...
	if err != nil {
		log.Printf("error: unable to read question paper: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "unable to access stored question paper"})
		return
	}
	...
	if err != nil {
		log.Printf("error: unable to read answer key: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "unable to access stored answer key"})
		return
	}
	...
	promptText := strings.TrimSpace(promptDoc.SystemPrompt)
	if promptText == "" {
		promptText = defaultPrompt()
	}

	constraintLines := []string{
		fmt.Sprintf("Assessment Title: %s", assessment.Title),
	...
	if assessment.CohortType != "" {
		constraintLines = append([]string{fmt.Sprintf("Education Context: %s", strings.Title(assessment.CohortType))}, constraintLines...)
	}
	if gradeLevel != "" {
		constraintLines = append([]string{fmt.Sprintf("Grade Level: %s", gradeLevel)}, constraintLines...)
	}
	...
	derivedMax := resolveMaxScore(aiResp.MaxScore, maxScore)
	...
	if breakdownMax > 0 {
		if derivedMax <= 0 || math.Abs(breakdownMax-derivedMax) > tolerance {
			log.Printf("warning: breakdown max total %.2f differs from overall max %.2f; adjusting", breakdownMax, derivedMax)
		...
		if derivedMax > 0 && breakdownScore > derivedMax {
			log.Printf("warning: breakdown score %.2f exceeds overall max %.2f; clamping", breakdownScore, derivedMax)
			breakdownScore = derivedMax
		}
	}
	if breakdownScore > 0 && math.Abs(derivedScore-breakdownScore) > tolerance {
		log.Printf("warning: AI overall score %.2f mismatches breakdown total %.2f; adjusting", derivedScore, breakdownScore)
	...
	submission := models.Submission{
		...
		Subject:          subject,
		Grade:            gradeLevel,
		CohortType:       assessment.CohortType,
