...
// GetGroupedSubmissions returns only existing bulk batches (no auto-grouping of single submissions)
func (h *BulkBatchHandler) GetGroupedSubmissions(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	// Fetch existing bulk batches only - don't auto-group individual submissions
