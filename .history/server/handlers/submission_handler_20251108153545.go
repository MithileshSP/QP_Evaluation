...
func (h *SubmissionHandler) UploadSubmission(c *gin.Context) {
	...
	// Validate assessment completeness and return helpful diagnostics on failure
	missing := make([]string, 0)
	if assessment.ID.IsZero() {
		missing = append(missing, "id")
	}
	if assessment.QuestionPaper == nil {
		missing = append(missing, "questionPaper")
	} else {
		if strings.TrimSpace(assessment.QuestionPaper.StoredPath) == "" {
			missing = append(missing, "questionPaper.storedPath")
		}
	}
	if assessment.AnswerKey == nil {
		missing = append(missing, "answerKey")
	} else {
		if strings.TrimSpace(assessment.AnswerKey.StoredPath) == "" {
			missing = append(missing, "answerKey.storedPath")
		}
	}
	if len(missing) > 0 {
		log.Printf("UploadBulkSubmissions: assessment incomplete or missing fields: %v", missing)
		c.JSON(http.StatusBadRequest, gin.H{
			"error":   "assessment is incomplete",
			"details": missing,
			"hint":    "Upload a question paper and wait for the auto-generated answer key to be saved before submitting student files.",
		})
		return
	}
...
func (h *SubmissionHandler) UploadBulkSubmissions(c *gin.Context) {
	...
	case len(errorsOut) > 0:
		...
		if bundleURL := h.tryGenerateBatchBundle(uploadsDir, titlePrefix, titleOverride, results, &assessment); bundleURL != "" {
...
// PreviewBatchExport extracts a batch zip to a previews folder and serves files inside it.
// This lets the teacher open the batch index.html in-browser and follow relative links.
func (h *SubmissionHandler) PreviewBatchExport(c *gin.Context) {
	fileName := strings.TrimSpace(c.Param("name"))
	if fileName == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "export name is required"})
		return
	...
	zipPath := filepath.Join(uploadsDir, "exports", "batches", fileName)
	...
	// preview directory per zip (strip extension)
	base := strings.TrimSuffix(fileName, filepath.Ext(fileName))
	previewRoot := filepath.Join(uploadsDir, "exports", "batches", "previews", base)

	// ensure preview extraction exists
	if _, err := os.Stat(previewRoot); os.IsNotExist(err) {
		if err := os.MkdirAll(previewRoot, 0o755); err != nil {
			log.Printf("error: unable to create preview dir %s: %v", previewRoot, err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "unable to prepare preview"})
			return
		}
		if err := extractZipToFolder(zipPath, previewRoot); err != nil {
			log.Printf("error: unable to extract preview %s: %v", zipPath, err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "unable to extract preview"})
			return
		}
	}

	// requested path inside preview (wildcard param includes leading /)
	reqPath := c.Param("path")
	if reqPath == "" || reqPath == "/" {
		// serve index.html at preview root (if exists)
		idx := filepath.Join(previewRoot, "index.html")
		if _, statErr := os.Stat(idx); statErr == nil {
			c.File(idx)
			return
		}
		// if no index, show a simple listing
		files, _ := os.ReadDir(previewRoot)
		names := make([]string, 0, len(files))
		for _, f := range files {
			names = append(names, f.Name())
		}
		c.JSON(http.StatusOK, gin.H{"previewRoot": base, "contents": names})
		return
	}

	// normalize and secure path
	cleaned := filepath.Clean(strings.TrimPrefix(reqPath, "/"))
	target := filepath.Join(previewRoot, cleaned)
	rel, err := filepath.Rel(previewRoot, target)
	...
	c.JSON(http.StatusNotFound, gin.H{"error": "file not found in preview"})
}
...
func (h *SubmissionHandler) generateBulkBundle(uploadsDir, batchLabel string, submissions []models.Submission, assessment *models.Assessment) (string, error) {
	...
	baseSlug := sanitizeFileName(label)
	if baseSlug == "" {
		baseSlug = "batch"
	}
	timestamp := time.Now().UTC().Unix()

	workspaceRoot := filepath.Join(uploadsDir, "exports", "batches", fmt.Sprintf("%s-%d", baseSlug, timestamp))
	contentRoot := filepath.Join(workspaceRoot, baseSlug)
	submissionsRoot := filepath.Join(contentRoot, "submissions")

	if err := os.MkdirAll(submissionsRoot, 0o755); err != nil {
		return "", err
	}

	defer func() {
		if _, err := os.Stat(workspaceRoot); err == nil {
			if remErr := os.RemoveAll(workspaceRoot); remErr != nil {
				log.Printf("warning: unable to clean batch workspace %s: %v", workspaceRoot, remErr)
			}
		}
	}()
	...
	for _, sub := range submissions {
		...
		targetDir := filepath.Join(submissionsRoot, subFolder)
		...
		reportRel := filepath.ToSlash(filepath.Join("submissions", subFolder, "index.html"))
		answerFile := sanitizeFileNamePreserveExt(sub.FileName)
		answerRel := ""
		if answerFile != "" {
			candidate := filepath.Join(targetDir, answerFile)
			if _, err := os.Stat(candidate); err == nil {
				answerRel = filepath.ToSlash(filepath.Join("submissions", subFolder, answerFile))
			}
		}
	...
	questionPaperRel := ""
	if assessment != nil && assessment.QuestionPaper != nil && assessment.QuestionPaper.StoredPath != "" {
		if _, err := os.Stat(assessment.QuestionPaper.StoredPath); err == nil {
			sanitized := sanitizeFileNamePreserveExt(assessment.QuestionPaper.FileName)
			if sanitized == "" {
				sanitized = filepath.Base(assessment.QuestionPaper.StoredPath)
			}
			qpFolder := filepath.Join(contentRoot, "question-paper")
			if err := os.MkdirAll(qpFolder, 0o755); err == nil {
				destPath := filepath.Join(qpFolder, sanitized)
				if copyErr := copyFile(assessment.QuestionPaper.StoredPath, destPath); copyErr == nil {
					questionPaperRel = filepath.ToSlash(filepath.Join("question-paper", sanitized))
				}
			}
		}
	}

	if err := os.WriteFile(filepath.Join(contentRoot, "index.html"), []byte(buildBatchHTML(label, rows, sumScore, sumMax, questionPaperRel)), 0o644); err != nil {
		return "", err
	}
	...
	batchJSON := map[string]any{
		...
		"totalScore":    sumScore,
		"totalMax":      sumMax,
		"questionPaper": questionPaperRel,
		"submissions":   rows,
	}
	...
	if err := os.WriteFile(filepath.Join(contentRoot, "overview.json"), jsonBytes, 0o644); err != nil {
		return "", err
	}

	zipName := fmt.Sprintf("%s-batch-%d.zip", baseSlug, timestamp)
	zipPath := filepath.Join(uploadsDir, "exports", "batches", zipName)
	if err := createZipFromFolder(zipPath, workspaceRoot, contentRoot); err != nil {
		return "", err
	}
...
func (h *SubmissionHandler) tryGenerateBatchBundle(uploadsDir, titlePrefix, titleOverride string, submissions []models.Submission, assessment *models.Assessment) string {
	if len(submissions) == 0 {
		return ""
	}
	...
	zipName, err := h.generateBulkBundle(uploadsDir, label, submissions, assessment)
	if err != nil {
...
func buildBatchHTML(batchLabel string, rows []batchSummaryRow, totalScore, totalMax float64, questionPaper string) string {
...
.summary{display:flex;flex-wrap:wrap;gap:1rem;margin-top:1rem;font-size:1rem;color:#475569;} .summary span{border-radius:999px;background:#eef2ff;border:1px solid #c7d2fe;padding:0.45rem 0.9rem;font-weight:600;}
	...
	builder.WriteString("</span></div>")
	if strings.TrimSpace(questionPaper) != "" {
		builder.WriteString("<p><strong>Question paper:</strong> <a href=\"")
		builder.WriteString(html.EscapeString(questionPaper))
		builder.WriteString("\" download>Download</a></p>")
	}

	builder.WriteString("<table><thead><tr><th>Submission</th><th>Answer Script</th><th>Score</th><th>Max</th><th>Evaluation</th></tr></thead><tbody>")
	for _, row := range rows {
		builder.WriteString("<tr><td>")
		builder.WriteString(html.EscapeString(fallbackString(row.Title, row.FileName)))
		builder.WriteString("</td><td>")
		if strings.TrimSpace(row.AnswerPath) != "" {
			builder.WriteString("<a href=\"")
			builder.WriteString(html.EscapeString(row.AnswerPath))
		...
		builder.WriteString(html.EscapeString(row.ReportPath))
		builder.WriteString("\">Open report</a></td></tr>")
	}
