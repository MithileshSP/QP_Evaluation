...
// GetGroupedSubmissions returns only explicitly created bulk batches (from bulk uploads)
// Does NOT auto-group individual submissions - they remain separate
func (h *BulkBatchHandler) GetGroupedSubmissions(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	// Fetch only existing bulk batches (created during bulk uploads)
	...
	// Return only real bulk batches - no auto-grouping
	c.JSON(http.StatusOK, existingBatches)
}
