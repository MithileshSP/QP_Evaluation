...
func Register(r *gin.Engine, prompt *handlers.PromptHandler, submissions *handlers.SubmissionHandler, assessments *handlers.AssessmentHandler, bulkBatches *handlers.BulkBatchHandler) {
	...
	api.GET("/bulk-batches", bulkBatches.GetAllBulkBatches)
	api.GET("/bulk-batches/grouped", bulkBatches.GetGroupedSubmissions) // Auto-grouped submissions
	api.POST("/bulk-batches", bulkBatches.CreateBulkBatch)
	api.GET("/bulk-batches/:id", bulkBatches.GetBulkBatch)
	api.DELETE("/bulk-batches/:id", bulkBatches.DeleteBulkBatch)
}
