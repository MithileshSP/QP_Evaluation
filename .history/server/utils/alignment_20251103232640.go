...
var criticalKeywords = map[string]struct{}{
	"python":      {},
	"javascript":  {},
	"typescript":  {},
	"java":        {},
...
func EvaluateAlignment(subject, submissionTitle, promptTitle, promptBody, transcript, reasoning string) string {
	...
	if len(keywordSet) == 1 && matched == 0 {
		if _, critical := criticalKeywords[keywordSet[0]]; critical {
			return buildWarning(keywordSet)
		}
	}
	...
	if subject != "" {
		...
		if len(subjectWords) >= 2 {
			...
			if len(missingSubject) == len(subjectWords) {
				return buildWarning(missingSubject)
			}
		}
	}
