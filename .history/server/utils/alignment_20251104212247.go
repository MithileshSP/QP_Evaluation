...
var mathEquationPattern = regexp.MustCompile(`\d\s*[*+\-×÷=]\s*\d`)
...
func EvaluateAlignment(subject string, _ string, _ string, _ string, transcript, reasoning string) string {
	...
	transcriptText := strings.ToLower(strings.TrimSpace(transcript))
	if verificationText == "" {
		return ""
	}
	...
	suggested := detectSubject(verificationText, canonical)
	if keywords, ok := subjectKeywords[canonical]; ok {
		if matchesSubject(canonical, verificationText, transcriptText) {
			return ""
		}
		return buildWarning(subjectDisplay, keywords, suggested)
	}
	...
	return buildWarning(subjectDisplay, subjectWords, suggested)
}
...
func matchesSubject(subject, verificationText, transcriptText string) bool {
	...
	switch subject {
	case "mathematics":
		if mathEquationPattern.MatchString(transcriptText) {
			return true
		}
		if digitPattern.MatchString(transcriptText) {
			return true
		}
	}
	return false
...
func buildWarning(subject string, keywords []string, suggested string) string {
	if len(keywords) == 0 {
		message := "The uploaded answers do not appear to match the selected subject. Adjust the subject, title, and prompt to match this file before resubmitting."
		if suggested != "" {
			message += " Detected keywords align more with " + suggested + "."
		}
		return message
	}
	limit := len(keywords)
	...
	message := "The uploaded answers do not mention keywords associated with " + subjectLabel + ", such as " + strings.Join(sample, ", ") + ". Adjust the subject, title, and prompt to match this file before resubmitting."
	if suggested != "" {
		message += " Detected keywords align more with " + suggested + "."
	}
	return message
...
func detectSubject(text string, current string) string {
	bestSubject := ""
	bestScore := 0
