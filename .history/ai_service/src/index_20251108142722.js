  ...
  if (mime.startsWith("application/json") || mime.startsWith("text/")) {
        ...
        try {
          ...
          try {
            ...
            console.warn("warning: attachment decode produced fallback raw text for", label);
          }
        }
    ...
    try {
      const partsSummary = requestBody.contents[0].parts.map((p) => {
        ...
        if (p.text) return `text:${String(p.text).slice(0, 40).replace(/\n/g, ' ')}${String(p.text).length>40?'…':''}`;
        return 'unknown';
      });
      console.log('Request parts summary:', JSON.stringify(partsSummary));
    } catch (e) {
      console.warn('warning: failed to summarize request parts', e?.message || e);
    }
