...
app.get("/models", async (_, res) => {
  ...
  try {
    ...
    for (const version of apiVersions) {
      ...
      payload[version] = models.map((model) => ({
        name: model.name,
        displayName: model.displayName,
        supportedGenerationMethods: model.supportedGenerationMethods,
        supportedInputModalities:
          model.supportedInputModalities || model.inputModalities,
      }));
    }
    res.json(payload);
  } catch (error) {
    console.error("error: unable to list gemini models", error.message);
...
async function callGemini(apiVersion, model, body) {
  ...
  const url = `${GEMINI_API_URL.replace(/\/$/, "")}/${apiVersion}/models/${modelId}:generateContent`;
...
async function listModels(apiVersion) {
  ...
  try {
    const url = `${GEMINI_API_URL.replace(/\/$/, "")}/${apiVersion}/models`;
...
function supportsPdfInput(model = {}) {
  const modalities =
    ...
    [];
  return modalities.some((item) =>
    String(item).toUpperCase().includes("FILE")
  );
}
