        ...
        console.log(
          ...
          JSON.stringify({
            ...
            hasInlineData: Boolean(
              requestBody?.contents?.[0]?.parts?.find((part) => part.inline_data)
            ),
...
function buildModelCandidates(preferred) {
  const list = [];
  ...
  if (preferred) {
    ...
    if (!preferred.endsWith("-001")) {
      pushUnique(`${preferred.replace(/^models\//, "models/")}-001`);
    }
  }

  DEFAULT_MODELS.forEach(pushUnique);
  return list;
}
