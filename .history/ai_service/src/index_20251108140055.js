    ...
    const partFor = (label, att) => {
      ...
      try {
        if (mime === 'application/json' || mime.startsWith('text/')) {
          // decode base64 into utf8 string and include as a text part
          const decoded = Buffer.from(att.data, 'base64').toString('utf8');
          // include a small preface so the model knows the role
          parts.push({ text: decoded });
          return parts;
        }
      } catch (e) {
        // fallback to inlineData when decoding fails
        console.warn('warning: unable to decode attachment for text embedding', e?.message || e);
      }
