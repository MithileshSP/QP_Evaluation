...
app.post("/assessments/analyze", async (req, res) => {
  try {
    ...
    if (rawText) {
      console.log(`Analysis raw response preview: ${rawText.substring(0, 500)}`);
    }
    ...
    const grade = coerceNonEmptyString(
    ...
    let maxScore = Number.isFinite(requestedMaxScore) && requestedMaxScore > 0
      ? requestedMaxScore
      : summedMaxScore;
...
function coerceQuestionSpecs(value) {
  ...
  return value
    .map((entry, index) => {
      ...
      const answer = coerceOptionalString(entry?.answer || entry?.solution, 1200);
      const marks = Number(
...
function normalizeCohortTypeField(value) {
  ...
  if (raw.includes("college") || raw.includes("engineering") || raw.includes("university")) {
    return "college";
  }
  return "school";
}
