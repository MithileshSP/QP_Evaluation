...
app.post("/evaluate", async (req, res) => {
  try {
    ...
    if (configuredModel) {
      ...
      console.log(`Using default Gemini model: ${DEFAULT_MODEL}`);
    }
    if (!modelsToTry.includes(DEFAULT_MODEL)) {
      if (configuredModel) {
        console.log(
          `Will fall back to default model ${DEFAULT_MODEL} if override fails`
        );
      }
    ...
    for (const modelName of modelsToTry) {
      for (const apiVersion of apiVersions) {
        try {
          ...
          if (status === 404) {
            console.warn(
              `warning: Gemini model "${modelName}" unavailable on ${apiVersion}: ${message}`
            );
