  ...
  const transientErrorCodes = new Set([
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        const message =
          error?.message ||
          (errorCode ? `transient network error (${errorCode})` : "network error");
        const isTransient =
...
async function attemptModelRequest(model, apiVersions, requestBody) {
  ...
  for (const apiVersion of apiVersions) {
    const maxAttempts = 2;
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      const attemptTag = maxAttempts > 1 ? ` (attempt ${attempt}/${maxAttempts})` : "";
    try {
      console.log(
          "calling Gemini",
          JSON.stringify({
  ...
  while (activeEvaluations >= maxConcurrent) {
    await sleep(100);
        ...
        if (transientErrorCodes.has(errorCode)) {
          const message =
            error?.message || `transient network error (${errorCode})`;
          console.warn(
            `warning: transient error contacting Gemini on ${apiVersion}${attemptTag}: ${message}`
          );
          lastMessage = message;
