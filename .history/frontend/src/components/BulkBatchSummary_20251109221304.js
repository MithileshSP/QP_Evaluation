...
import { generateDetailedBulkExcel } from "../utils/excelExport";
import { deleteBulkBatch } from "../api/bulkBatches";
...
export default function BulkBatchSummary({ batch, onClear, className }) {
  ...
  const handleExcelExport = () => {
    try {
      const filename = generateDetailedBulkExcel(batch);
  ...
  const handleDismiss = async () => {
    // Try to delete from backend if batch has a valid ID
    if (batch?.id || batch?._id) {
      try {
        const batchId = batch.id || batch._id;
        await deleteBulkBatch(batchId);
        console.log("Batch deleted from backend:", batchId);
      } catch (error) {
        console.error("Failed to delete batch from backend:", error);
        // Continue with frontend removal even if backend fails
      }
    }
    // Always call onClear to remove from frontend state
    onClear();
  };
  ...
  return (
    <section className={`panel panel--workspace ${className || ""}`}>
