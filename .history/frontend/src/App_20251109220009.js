...
import { fetchBulkBatches } from "./api/bulkBatches";
...
function App() {
  ...
  useEffect(() => {
    const loadWorkspace = async () => {
      const [assessmentResult, promptResult, submissionsResult, bulkBatchesResult] =
      ...
      if (bulkBatchesResult.status === "fulfilled") {
        setBulkBatches(bulkBatchesResult.value?.data || []);
      } else {
        console.error(
          "error: unable to load bulk batches",
          bulkBatchesResult.reason
        );
      }
