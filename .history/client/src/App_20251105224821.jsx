...
function App() {
  ...
  const handleSubmission = async (event) => {
    ...
    setUploadState({ uploading: true, error: null, message: null });
    ...
    try {
      ...
      if (!isBulk) {
        ...
        setUploadState({
          ...
          error: null,
          message:
            "Submission evaluated successfully. Download the bundle from the workspace card.",
        });
        return;
      }
      ...
      const successMessage =
        created.length > 0
          ? `Evaluated ${created.length} submission${
              created.length === 1 ? "" : "s"
            }. Download the bundles from the history panel.`
          : null;
      const errorMessage =
        failures.length > 0
      ...
      setUploadState({ uploading: false, error: message, message: null });
    }
  };
