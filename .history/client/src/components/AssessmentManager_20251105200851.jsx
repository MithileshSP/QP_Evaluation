...
const COHORT_CHOICES = [
  ...
  { value: "college", label: "Engineering college" },
];

const SCHOOL_GRADE_OPTIONS = Array.from({ length: 12 }, (_, index) => {
...
const getGradeOptions = (type) =>
  type === "college" ? ENGINEERING_OPTIONS : SCHOOL_GRADE_OPTIONS;

const formatCohortType = (value) => {
  if (value === "college") {
...
export default function AssessmentManager({ assessment, onSaved, className }) {
  ...
  useEffect(() => {
    if (!assessment) {
      ...
      setForm((prev) => ({ ...prev, grade: gradeOptions[0]?.value || "" }));
      return;
    }

    const detectedType = assessment.cohortType === "college" ? "college" : "school";
    const nextGradeOptions = getGradeOptions(detectedType);
    const resolvedGrade = assessment.grade && nextGradeOptions.some((option) => option.value === assessment.grade)
      ? assessment.grade
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const resolvedGrade = data.grade && savedOptions.some((option) => option.value === data.grade)
        ? data.grade
