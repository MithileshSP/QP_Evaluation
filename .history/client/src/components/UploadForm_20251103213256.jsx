...
export default function UploadForm({
  ...
  const selectedSubject = useMemo(() => {
    if (form.subject === "custom") {
      return {
        value: "custom",
        label: form.customSubject?.trim() || "Custom subject",
      };
    }
    return (
      SUBJECT_OPTIONS.find((option) => option.value === form.subject) ||
      SUBJECT_OPTIONS[0]
    );
    ...
    <section className="panel">
      <header className="panel__heading">
        ...
        <p className="panel__meta">
          Attach a clear image (JPG/PNG) or PDF export. The AI will grade it
          using your latest prompt.
        </p>
      </header>
      ...
      <form className="form" onSubmit={onSubmit}>
          ...
          <select
            ...
            onChange={(event) =>
              setForm((prev) => ({
                ...
                customSubject:
                  event.target.value === "custom" ? prev.customSubject : "",
              }))
            }
            required
          >
        ...
        {form.subject === "custom" && (
          <label className="form__label" htmlFor="submissionCustomSubject">
            Custom subject
            <input
              id="submissionCustomSubject"
              name="customSubject"
            ...
            <p className="form__hint">
              Provide the subject name you want to appear on the evaluation.
            </p>
          </label>
        )}
          ...
          <input
            ...
            placeholder={`e.g. ${
              selectedSubject.value
                ? `${selectedSubject.label} Unit Test`
                : "Mid Term Evaluation"
            }`}
            value={form.title}
            onChange={(event) =>
