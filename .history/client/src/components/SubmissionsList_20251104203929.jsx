              ...
              >
                ...
                {Array.isArray(submission.breakdown) &&
                  submission.breakdown.length > 0 && (
                    <details className="submission-card__breakdown" open>
                      <summary>Question breakdown</summary>
                      <ul>
                        {submission.breakdown.map((item) => (
                          <li
                            key={`${submission.id || submission._id}-${item.questionNumber || "na"}`}
                          >
                            <strong>{item.questionNumber || "Question"}</strong>
                            : {Number(item.score).toFixed(1)} / {" "}
                            {Number(item.maxScore).toFixed(1)} — {item.reason || "No notes"}
                          </li>
                        ))}
                      </ul>
                    </details>
                ...
                {transcript && (
                  <details className="submission-card__transcript" open>
