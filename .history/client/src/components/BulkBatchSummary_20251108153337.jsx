        ...
        <div className="bulk-folder__items">
          {submissions.length === 0 ? (
            ...
            submissions.map((submission, index) => {
              const submissionId = submission.id || submission._id || `${batch.id || "batch"}-${index}`;
              const isOpen = expandedId === submissionId;
              const { score, maxScore } = resolveSubmissionScore(submission);
              return (
                <div
                  key={submissionId}
                  className={`bulk-folder__item${isOpen ? " bulk-folder__item--open" : ""}`}
                >
                  <button
                    type="button"
                  ...
                  >
                    <div className="bulk-folder__item-text">
                      ...
                      <span className="bulk-folder__item-file">{submission.fileName}</span>
                    </div>
                    <div className="bulk-folder__item-meta">
                      <span className="bulk-folder__item-score">
                        {formatScore(score)} / {formatScore(maxScore)} marks
                      </span>
                      <span className="bulk-folder__item-chevron" aria-hidden="true" />
                    </div>
                  </button>
                  {isOpen && (
