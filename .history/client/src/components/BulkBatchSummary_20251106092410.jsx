...
function BatchModal({ batch, onClose }) {
  ...
  return (
    <div className="modal" role="dialog" aria-modal="true">
      <button className="modal__overlay" type="button" onClick={handleCloseModal}>
        ...
        <header className="modal__header">
          <div>
            ...
            <p className="modal__subtitle">
              {batch.submissions.length} submission{batch.submissions.length === 1 ? "" : "s"}
            </p>
          </div>
          <button className="button button--ghost" type="button" onClick={handleCloseModal}>
            Close
          </button>
        </header>
...
export default function BulkBatchSummary({ batch, onClear }) {
  ...
  return (
    <section className="panel panel--workspace">
      ...
      {summary?.context && (
        <p className="panel__meta">{summary.context}</p>
      )}
      {summary?.updatedAt && (
